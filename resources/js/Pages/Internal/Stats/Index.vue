...
function destroy(stat) {
    if (confirm(`Delete stat "${stat.label}"? This cannot be undone.`)) {
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-warm-500 mt-0.5">Manage the key figures shown across the site</p>
                </div>
        ...
        <div v-if="!stats?.data?.length"
            ...
            <p class="text-sm text-warm-500 mb-5">Add your first key figure to highlight on the site.</p>
            <Link href="/internal/stats/create"
        ...
        <div v-else class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            <div v-for="stat in stats.data" :key="stat.id"
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-2xl font-bold font-display text-warm-900 truncate">{{ stat.value }}</p>
                    <p class="text-sm text-warm-500 truncate">{{ stat.label }}</p>
                </div>
